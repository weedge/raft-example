...
func main() {
	...
	if err != nil {
		store.log.Fatalf(fmt.Sprintf("listen %s failed: %s", store.opts.httpAddress, err))
	}
	...
	if err != nil {
		store.log.Fatalf(fmt.Sprintf("new raftNode node failed:%v", err))
	}
	store.raftNode = raft
	...
	if store.opts.joinAddress != "" {
		...
		if err != nil {
			store.log.Fatalf(fmt.Sprintf("join raftNode cluster failed:%v", err))
		}
	}
