...
import (
	...
	raftpebble "github.com/weedge/raft-pebble"
)
...
func newRaftNode(opts *Options, ctx *KVStoreContext) (*RaftNode, error) {
	...
	logStore, stableStore, err := getLogStore(opts, ctx)
	if err != nil {
		return nil, err
	}
	...
	//fmt.Printf("raftConf: %+v", raftConfig)
...
func getLogStore(opts *Options, _ *KVStoreContext) (logStore raft.LogStore, stableStore raft.StableStore, err error) {
	switch opts.store {
	case "boltdb":
	...
	case "pebble":
		pebbleStore, er := raftpebble.New(raftpebble.WithDbDirPath(filepath.Join(opts.dataDir, "pebble")))
		if er != nil {
			err = er
			return
		}
		logStore = pebbleStore
		stableStore = pebbleStore
	default:
		err = errors.New("unsupport db raft log store")
		return
	}
